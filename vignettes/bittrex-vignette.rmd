...
including price and volume data. The ```bt_getmarkethistory``` extracts the last 
...
btc_eth_req = bt_getorderbook("btc-eth")$result
...
Trades are initiated using either the ```bt_buy``` or ```bt_sell``` functions. These functions return
